...
Windows 上也可以直接双击根目录的 `start-workbench.bat` 启动工作台。看到终端窗口中出现 `Ready` 后，再点击插件里的“打开工作台”。如果本地服务未启动，插件会停留在弹窗内提示“本地工作台未启动”，不会再直接跳到浏览器错误页。
...
手机不能直接运行这个 Chrome MV3 插件，也不能在手机上执行 BOSS 自动采集和自动打招呼。当前可靠用法是：

1. 电脑保持 `npm run dev` 或 `start-workbench.bat` 运行。
2. 手机和电脑连接同一个局域网。
3. 在电脑启动日志里查看 `Network` 地址，例如 `http://192.168.x.x:3000`，用手机浏览器打开这个地址查看工作台。

手机端适合查看岗位库、审批队列、筛选设置和运行记录；采集岗位、打开 BOSS 页面、执行已审批任务仍需要电脑 Chrome 插件完成。不要把工作台端口暴露到公网，里面包含本地求职数据。
